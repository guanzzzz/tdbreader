...
def type_transfer_namespace_ghser(FUNCTION_OF_DATABASE):
    if isinstance(FUNCTION_OF_DATABASE, dict):
        ...
        namespace_ghser = namespace_ghser.format(STATIC_DOUBLE=static_doubles)
        namespace_ghser = namespace_ghser.replace('(', '{').replace(')', '}').replace('[', '(').replace(']', ')') \
            .strip('}').strip()
        return namespace_ghser


def type_transfer_namespace_sys(ELEMENTS_OF_DATABASE, PHASE_OF_DATABASE, FUNCTION_OF_DATABASE,
                                CONSTITUENTS_OF_DATABASE, PARAMETERS_OF_DATABASE, TYPE_SWITCH=True):
    namespace_sys = \
    ...
    if isinstance(PHASE_OF_DATABASE, dict):
        ...
        for i in range(len(phases)):
            ...
            for key in sub_phase_list:
                ...
                for j in range(len(transformed_expression_list)):
                    ...
                    piecewise_expressions_subphase += piecewise_expression_subphase.format(
                        ...
                        T_MAX=range_list[j + 1], TRANSFORMED_PIECE_EXPRESSION=transformed_expression_list[j])

                expression_subphase = expression_subphase.format(T_EXTRA_MIN=range_list[0], T_EXTRA_MAX=range_list[-1],
                ...
                function_name_subphase = key[2].type + '_' + str(key[2].source_phase_name) + '_' + \
                    function_name_constituents_transfromed + str(key[2].order)
                ...
                static_doubles += static_double_subphase.format(FUNCTION_NAME=function_name_subphase,
                                                                EXPRESSION=expression_subphase)
            separator = \
                '               //---------------------------------------------------------------------\n' \
                '               //{DEFINITION_BELOW_NAME}\n' \
                '               //---------------------------------------------------------------------\n'
            ...
            if len(subphase_sublattce_subsublattice) == 1:
                # print('1111111111111111',subphase_sublattce_subsublattice)
                ...
                for j in range(len(subphase_sublattce_subsublattice[0])):
                    ...
                    site_doubles += site_double.format(SITE_NAME=site_double_names_x[j],
                                                       SOURCE_SUBLATTICE=site_double_names_x[j])
                ...
                '''
                static_double_gibbs_x_transformed = static_double_gibbs_x.format(
                    GIBBS_FUNCTION_NAME=subphase_gibbs_energy_name,
                    ORDER='x',
                ...
                static_doubles = static_doubles.replace('(', '{').replace(')', '}').replace('[', '(').replace(']', ')')
                ...
                # subval_subval.extend(site_latter_expression)

                # site_latter_expression
                # print(subval_subval)
                # print('site_double_names:', site_double_names)
                ...
                # print('subval_subval', subval_subval)

                site_double_first_order_derivatives_not_deleted = []
                # print('site_double_subnames:', site_double_subnames)
                # print('subphase_gibbs_energy_name', subphase_gibbs_energy_name)
                for j in site_double_subnames:
                    ...
                    for k in subval_subval:
                        ...
                        if symbol_x not in k:
                            ...
                            # symbol_log_x_count = k.count(symbol_log_x)
                            ...
                            # pprint(site_double_first_order_derivatives_subexpression)
                    ...
                    # pprint(site_double_first_order_derivatives)

                    site_latter_expression_first_order_derivatives = []
                    for k in range(len(site_latter_expression)):
                        # site_latter_expression_first_order_derivatives_subexpression = []
                        # print(site_latter_expression[k])
                        # print(symbol_x)
                        if '{}*{}'.format(symbol_x, symbol_log_x) in site_latter_expression[k]:
                            ...
                            #site_latter_expression_first_order_derivatives.append(site_latter_expression[k].replace(
                            #    '{}*{}'.format(symbol_x, symbol_log_x), '1+{}'.format(symbol_log_x)))
                        else:

                            site_latter_expression_first_order_derivatives.append('0')
                            #site_latter_expression_first_order_derivatives.append(site_latter_expression[k])
                        # site_latter_expression_first_order_derivatives.append(site_latter_expression_first_order_derivatives_subexpression)
                    ...
                    site_double_first_order_derivatives += \
                        val_expression_latter.format(PROPORTION_X_SITE=subphase_proportion_transformed[0],
                                                     X_SITE_LATTER_EXPRESSION=
                                                     '('+site_latter_expression_first_order_derivatives[0]+')',
                                                     PROPORTION_Y_SITE=subphase_proportion_transformed[1],
                                                     Y_SITE_LATTER_EXPRESSION=
                                                     '('+site_latter_expression_first_order_derivatives[1]+')')

                    site_double_first_order_derivatives_untransformed = \
                        '                       double val = {SITE_DOUBLE_FIRST_ORDER_DERIVATIVES_LATTER}'

                    site_double_first_order_derivatives_subtransformed = site_double_first_order_derivatives.strip('+')
                    ...
                    site_double_first_order_derivatives_transformed = \
                        site_double_first_order_derivatives_untransformed.format(
                            SITE_DOUBLE_FIRST_ORDER_DERIVATIVES_LATTER=
                ...
                # print(site_double_second_order_names)
                ...
                for j in range(len(site_double_second_order_names_transformed)):
                    ...
                    second_order_derivatives_doubles += second_order_derivatives_double.format(
                        SECOND_ORDER_DERIVATIVES_NAMES=subphase_second_order_derivatives_name
                                                       + site_double_second_order_names_transformed[j],
                        SITE_DOUBLES=site_doubles,
                ...
                # print('3333333333333333',subphase_sublattce_subsublattice)
                pass
            else:
                pass
...
def type_transfer_namespace_TDB(namespace_ghser, namespace_sys):
    namespace_tdb = \
        ...
        '\n' \
        ']'
    return namespace_tdb.format(NAMESPACE_GHSER=namespace_ghser,
                                NAMESPACE_SYS=namespace_sys).replace('[', '{').replace(']', '}')


if __name__ == "__main__":
    tdb = open("Sn-Ag-Bi.TDB", encoding='utf-8')
    tdb_file = Database(tdb)
    tdb_file.configuration()
    type_transfer_namespace_sys(tdb_file.ELEMENTS, tdb_file.PHASES, tdb_file.FUNCTIONS,
